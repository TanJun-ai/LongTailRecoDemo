...
from metaga_training import model_training
...
training_query_size = len(training_supp_dataset.trainUniqueItems)   # 训练集有252条数据
print("===========training_query_size===========")
print(training_query_size)
train_items_id_list = []
train_users_id_list = []
train_labels_list = []
training_query_path = args.data_dir + '/training/top_query_14.txt'
training_query_label_path = args.data_dir + '/training/top_query_14_label.txt'
'''获取item_id_list和user_id_list'''
for i in range(training_query_size):
    cache_list = []
    with open(training_query_path) as f:
...
'''获取label_list'''
for j in range(training_query_size):
    cache_list = []
    with open(training_query_label_path) as f:
        for l in f.readlines():
            if len(l) > 0:
                l = l.strip('\n').split(' ')
                cache_list.append(l)
...
testing_query_size = len(testing_supp_dataset.trainUniqueItems)   # 测试集有1011条数据
print("===========testing_query_size============")
print(testing_query_size)
test_items_id_list = []
test_users_id_list = []
test_labels_list = []
testing_query_path = args.data_dir + '/testing/tail_query_14.txt'
testing_query_label_path = args.data_dir + '/testing/tail_query_14_label.txt'

for i in range(testing_query_size):
    cache_list = []
    with open(testing_query_path) as f:
        for l in f.readlines():
            if len(l) > 0:
                l = l.strip('\n').split(' ')
                cache_list.append(l)
...
for j in range(testing_query_size):
    cache_list = []
    with open(testing_query_label_path) as f:
        for l in f.readlines():
            if len(l) > 0:
                l = l.strip('\n').split(' ')
                cache_list.append(l)
...
def model_test_training(args, training_supp_dataset, testing_supp_dataset, Recmodel, bpr):
    ...
    for j in range(len(test_item_index)):   # 400次循环
        ...
        for t in range(len(test_list)):     # 给14个user补充负样本
            ...
            # if t < (len(test_list)/2):
            #     neg_user_emb_list.append(test_users_gcn[neg_user].tolist())
            # else:
            #     neg_user_emb_list.append((sim_score_list[t-7]*test_users_gcn[neg_user]).tolist())
...
for epoch in range(args.epochs):
    ...
    training_set_loss = model_training(args, training_supp_dataset, Recmodel, bpr, "train-set")

    train_loss = []
    for j in range(training_query_size):
        train_items_id_list[j] = train_items_id_list[j]
        train_users_id_list[j] = train_users_id_list[j]
        train_labels_list[j] = train_labels_list[j]

        training_query_y_pred = Recmodel(train_items_id_list[j], train_users_id_list[j], "train-set")
        loss = F.mse_loss(training_query_y_pred.to(args.mdevice), train_labels_list[j].float().view(-1, 1).to(args.mdevice))
        train_loss.append(loss)
    t_loss = torch.stack(train_loss).mean(0)
    # t_loss = t_loss + torch.tensor(training_set_loss)    # 加上训练集上的loss进行更新
    ...
    # testing_aver_loss = model_training(args, testing_supp_dataset, Recmodel, bpr, "test-set")
    ...
    for j in range(testing_query_size):
        ...
        testing_query_y_pred = Recmodel(test_items_id_list[j], test_users_id_list[j], "test-set")
        test_loss = F.mse_loss(testing_query_y_pred.to(args.mdevice), test_labels_list[j].float().view(-1, 1).to(args.mdevice))
        te_loss.append(test_loss)
        test_output_list, query_recom_list = testing_query_y_pred.view(-1).sort(descending=True)
        test_prec10, test_ndcg10 = add_metric(query_recom_list, test_labels_list[j], topn=10)
        test_prec8, test_ndcg8 = add_metric(query_recom_list, test_labels_list[j], topn=8)
        test_prec5, test_ndcg5= add_metric(query_recom_list, test_labels_list[j], topn=5)
    ...
    total_loss = torch.stack(te_loss).mean(0)
    test_end = time()
    # print(f'EPOCH[{epoch + 1}/{args.epochs}] Test-Loss[{total_loss:.3f}] Time[{(test_end-test_start):.3f}]'
    #       f'Prec[{total_prec:.4f}] ndcg[{total_ndcg:.4f}]')
