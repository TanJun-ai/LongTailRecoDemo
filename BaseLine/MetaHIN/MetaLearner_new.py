...
class MetaLearner(torch.nn.Module):
    def __init__(self, config):
        ...
        self.fc1_in_dim = 16 + config['item_embedding_dim']     # 16+16=32
        self.fc2_in_dim = config['first_fc_hidden_dim']     # 64
        self.fc2_out_dim = config['second_fc_hidden_dim']   # 64
        self.config = config
        ...
        w1 = torch.nn.Parameter(torch.ones([self.fc2_in_dim, self.fc1_in_dim]))  # 64, 48
...
class MetapathLearner(torch.nn.Module):
    def __init__(self, config):
        ...
        self.vars = torch.nn.ParameterDict()
        neigh_w = torch.nn.Parameter(torch.ones([16, config['item_embedding_dim']]))  # dim=32, movielens 0.81006
        torch.nn.init.xavier_normal_(neigh_w)
        self.vars['neigh_w'] = neigh_w
        self.vars['neigh_b'] = torch.nn.Parameter(torch.zeros(16))

    def forward(self, user_emb, item_emb, neighs_emb, index_list, vars_dict=None):
