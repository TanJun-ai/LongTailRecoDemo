...
class Scheduler(nn.Module):
    def __init__(self, N, args, grad_indexes, use_deepsets=True):
    ...
    def sample_task(self, prob, size, replace=True):
        ...
        if len(np.where(p > 0)[0]) < size:
            ...
            actions = np.random.choice(np.arange(len(prob)), p=p / np.sum(p), size=size,
                                       replace=replace)
            actions = [torch.tensor(x).to(self.args.mdevice) for x in actions]
    ...
    def get_weight(self, batch_support, batch_query, model, pt):
        ...
        task_layer_inputs = [torch.stack(input_embedding_norm).to(self.args.mdevice),
                             torch.stack(input_embedding_cos).to(self.args.mdevice)]

        weight = self.forward(task_losses, task_layer_inputs,
                              torch.tensor([pt]).long().repeat(len(task_losses)).to(self.args.mdevice))
